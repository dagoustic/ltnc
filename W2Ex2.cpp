...
string kangaroo(int x1, int v1, int x2, int v2) {
    ...
    else if (x1==x2 and v1==v2) return "YES";
    else if (x1!=x2 and v1==v2) return "NO";
    else {
        int delta_x=x2-x1;
        int delta_v=v1-v2;
        int jump=delta_x/delta_v;
        if (delta_x/delta_v<0) return "NO";
        else {
            if (delta_v*jump==delta_x) return "YES";
            else return "NO";
        }
    }
}
